...
def tanhTransPrime(x):
    return 1-np.tanh(x)**2;
    
def sigmoidTrans(x):#corresponds to MATLAB logsig
    return np.divide(1.0,(1.0+np.exp(-x)))
    
def sigmoidTransPrime(x):
    return np.divide(np.exp(-x),np.power(1.0+np.exp(-x),2));
    
def ReLUTrans(x):#Rectified Linear Unit
    x[x<0.0] = 0.0
    return x
    
def ReLUTransPrime(x):
    return (x < 0).astype(float)
    
def ELUTrans(x):#Exponential Linear Unit (with a fixed as 1.0 - smooth curve)
    x[x<0.0] = 1.0*np.expm1(x[x<0.0])
    return x
    
def ELUTransPrime(x):
    x[x<0.0] = 1.0*np.exp(x[x<0.0])
    x[x>=0.0]= 1.0
    return x

def CreateNN(nn,dIn=[0],dIntern=[],dOut=[],transfers=None):
    ...
    Args:
        ...
        transfers: list of transfer functions for hidden layers
    Returns:
    ...
    if transfers is None:
        print('transfer functions undefined, using tanh for all')
        net['transfers'] = [tanhTrans for ind in range(net['M']-1)]
        net['transfersPrime'] = [tanhTransPrime for ind in range(net['M']-1)]
    elif len(transfers) == 1:
        trFunName = transfers[0].__name__
        print('using %s as transfer function for all'%trFunName)
        net['transfers'] = [transfers[0] for ind in range(net['M']-1)]
        net['transfersPrime'] = [globals()[trFunName + 'Prime'] for ind in range(net['M']-1)]
    elif len(transfers) == net['M']-1:
        net['transfers'] = []
        net['transfersPrime'] = []
        for ind in range(net['M']-1):
            trFunName = transfers[ind].__name__
            print('for layer %d, using transfer function %s'%(ind,trFunName))
            net['transfers'].append(transfers[ind])
            net['transfersPrime'].append(globals()[trFunName + 'Prime'])
    else:
        raise Exception("wrong specification of transfer functions")
...
def NNOut_(P,net,IW,LW,b,a={},q0=0):
    ...
    transfers = net['transfers'] #transfer functions
    ...
    #for all datapoints
        ...
        #for all layers m
            ...
            if m==M:
                ...
                #~ a[q,m] = np.tanh(n[q,m])
                #~ a[q,m] = tanhTrans(n[q,m])
                #~ a[q,m] = sigmoidTrans(n[q,m])
                a[q,m] = transfers[m-1](n[q,m])
        Y_NN[:,q-1]=a[q,M]
    Y_NN = Y_NN[:,q0:]
    return Y_NN,n,a
...
def RTRL(net,data):
    ...
    transfersPrime = net['transfersPrime']
        ...
        # decrement m in backpropagation order
            ...
            for u in U_:
                ...
                for l in L_b[m]:
                    ...
                    #~ S[q,u,m] = S[q,u,m] \
                        #~ + np.dot(np.dot(S[q,u,l],LW[l,m,0]),np.diag(tanhTransPrime(n[q,m])))
                    S[q,u,m] = S[q,u,m] \
                        + np.dot(np.dot(S[q,u,l],LW[l,m,0]),np.diag(transfersPrime[m-1](n[q,m])))
            ...
            if m in U:
                if m==M:
                    ...
                    S[q,m,m] = np.diag(transfersPrime[m-1](n[q,m])) #Sensitivity Matrix S[m,m]
...
#~ def onlyErrorCalc(net,data):
    #~ """ Should compute only error between input and output. returns error
        #~ 
    #~ Args:
        #~ net:    neural network
        #~ data:   Training Data
...
def train_LM(P,Y,net,k_max=100,E_stop=1e-10,RelE_stop=1e-10,dampfac=3.0,dampconst=10.0,\
    ...
    J,E,e = RTRL(net,data)
    k = 0
    ErrorHistory=np.zeros(k_max+1) #Vektor for Error history
    ErrorHistory[k]=E
    RelErrorHistory=np.zeros(k_max+1) #Vektor for Relative Error history
    firstE=E
    RelE = E/firstE
    RelErrorHistory[k]=1.0
    if verbose:
        print('Iteration: ',k,'     Error: ',E,'     RelError: ',RelE,'    scale factor: ',dampfac)
    ...
    #run loop until either k_max or E_stop is reached
        ...
        if k>=k_max:
            ...
            print('Termination RelError reached')
            break
        elif early>=5.0:
    ...
    net['RelErrorHistory'] = RelErrorHistory[:k]
    return net
    
def train_LMWithValData(P,Y,PVal,YVal,net,k_max=100,E_stop=1e-10,RelE_stop=1e-10,dampfac=3.0,dampconst=10.0,\
            verbose = False,min_E_step=1e-09,maxValFails=5):
    """ Implementation of the Levenberg-Marquardt-Algorithm (LM) based on:
        Levenberg, K.: A Method for the Solution of Certain Problems in Least Squares.
        Quarterly of Applied Mathematics, 2:164-168, 1944.
        and
    ...
    #create data dict
    data,net = prepare_data(P,Y,net)
    dataVal,_= prepare_data(PVal,YVal,net)
    ...
    ErrorHistoryVal=np.zeros(k_max+1) #Vektor for Error history
    EVal = calc_error(net,dataVal)
    ErrorHistoryVal[k]=E
    RelErrorHistoryVal=np.zeros(k_max+1) #Vektor for Relative Error history
    firstEVal=EVal
    RelEVal = EVal/firstEVal
    RelErrorHistoryVal[k]=1.0
    
    if verbose:
        #~ print('Iteration: ',k,'     Error: ',E,'     RelError: ',RelE,'    scale factor: ',dampfac)
            print("Iteration:\t%d\tError:\t%10.5g\tRelError:\t%10.5g\tErrorVal:\t%10.5g\tRelErrorVal:\t%10.5g\tscale factor:\t%10.5g"%(k,E,RelE,EVal,RelEVal,dampfac)
        )
    ...
    #run loop until either k_max or E_stop is reached
                ...
                if abs(E-Enew)<=min_E_step:
                    ...
                    if verbose:
                        ...
                        if early>=5.0:
                            print('5 Times * E-Enew<=min_E_step Encountered!!')
                    break
            EnewVal = calc_error(net,dataVal) #calculate new validation Error E          
            RelEnewVal = EnewVal/firstEVal
        ...
        EVal = calc_error(net,dataVal)
        ErrorHistoryVal[k] = EVal
        RelEVal = EVal/firstEVal
        RelErrorHistoryVal[k] = RelEVal
        
        if verbose:
            #~ print('Iteration: ',k,'     Error: ',E,'     RelError: ',RelE,'    scale factor: ',dampfac)
                print("Iteration:\t%d\tError:\t%10.5g\tRelError:\t%10.5g\tErrorVal:\t%10.5g\tRelErrorVal:\t%10.5g\tscale factor:\t%10.5g\tvalFail:\t%d"%(k,E,RelE,EVal,RelEVal,dampfac,valFail)
            )
            
        if k > 1:
            if RelErrorHistoryVal[k-1] < RelErrorHistoryVal[k]:
                valFail += 1
            else:
                retNet = copy.deepcopy(net)                             # I will update the net only if the error on val data is improved
                # Note: I take advantage of assumption of net being small
                
        if valFail > maxValFails:
            print('Validation check fail -> terminating')
            break
    ...
    net['ErrorHistoryVal'] = ErrorHistoryVal[:k]
    net['RelErrorHistoryVal'] = RelErrorHistoryVal[:k]
